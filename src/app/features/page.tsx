...
import { FeatureSection, type FeatureGroup } from "@/components/features/FeatureSection";
import { DocumentBand } from "@/components/visuals/DocumentBand";
...
const TONES = ["ground", "surface", "ground", "surface", "ground", "dark"] as const;
...
export default function FeaturesPage() {
  return (
    <>
      ...
      <main id="main">
        {/* No side padding on the header itself: the chip band runs past both edges, so the text
            carries its own gutter instead. */}
        <header className="flex flex-col items-center gap-[22px] border-b border-border bg-surface pb-[72px] pt-[84px]">
          <span className="px-[var(--gutter)] font-mono text-label font-medium leading-[14px] tracking-mono text-faint">FEATURES</span>
          <h1 className="max-w-[900px] px-[var(--gutter)] text-center text-[clamp(34px,4vw,52px)] font-bold leading-[1.12] tracking-[-0.032em] text-text">
            Simple, secure records management
          </h1>
          <p className="max-w-[680px] px-[var(--gutter)] text-center text-copy leading-copy text-muted">
            One place for everything. Easy to find. Nothing expires by surprise. And it never leaves your house.
          </p>
          <div className="flex w-full flex-col items-center gap-[18px] pt-[34px]">
            <DocumentBand />
            <p className="px-[var(--gutter)] text-center font-mono text-label font-medium leading-[18px] tracking-mono text-faint">
        ...
        {GROUPS.slice(0, 3).map((group, i) => (
          <FeatureSection key={group.id} group={group} tone={TONES[i]} />
        ))}

        <section className="gutter flex flex-col items-center gap-[30px] border-t border-border bg-ground pb-[96px] pt-[84px]">
          <div className="flex max-w-[720px] flex-col items-center gap-[12px]">
            <span className="font-mono text-[12px] font-medium leading-[16px] tracking-mono text-faint">
              ONE DOCUMENT, EVERYTHING ABOUT IT
            </span>
          ...
          <div className="w-full max-w-[1000px] overflow-hidden rounded-[14px] border border-border">
            <Image
              ...
              width={1000}
              height={720}
              sizes="(min-width: 1100px) 1000px, calc(100vw - 40px)"
        ...
        {GROUPS.slice(3).map((group, i) => (
          <FeatureSection key={group.id} group={group} tone={TONES[i + 3]} />
        ))}
