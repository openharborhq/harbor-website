...
export function FeatureBand({ group, tone }: { group: FeatureGroup; tone: "ground" | "surface" }) {
  return (
     ...
     *
     * The offset is the inner group's own `delay`, not the outer `stagger`. A nested group has no
     * animatable values of its own, so the parent's positional stagger lands on nothing and its
     * children start from the parent's clock: measured, the first item moved in lockstep with the
     * left column, 21.1px at both, while the second and third staggered correctly behind it. A
     * `delay` on the group that actually owns the children is the offset that holds.
     */
    <StaggerGroup
      as="section"
      id={group.id}
    ...
    >
      ...
      <StaggerGroup as="ul" delay={0.24} className="grid min-w-0 flex-1 gap-x-[40px] gap-y-[40px] sm:grid-cols-2">
        {group.items.map((item) => (
          <StaggerItem as="li" key={item.title} className="flex min-w-0 flex-col gap-[12px]">
